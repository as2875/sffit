...
def calc_f_gemmi(structures, it92, bounds, nsamples):
    if nsamples % 2 == 0:
        f_calc = np.empty(
    ...
    for ind, st in enumerate(structures):
        ...
        f_calc[ind] = np.fft.rfftn(dc.grid.array)
...
def calc_cov(params, freq, dose):
    ...
    def one_bin(tree):
        ...
        return mat + noise * jnp.identity(len(mat))

    parsp = jax.tree.map(jax.nn.softplus, params)
    t1, t2 = jnp.meshgrid(dose, dose, indexing="xy")
    covmats = jax.lax.map(one_bin, (parsp["power"], parsp["noise"], freq))
    return covmats
    ...
    cov_calc = calc_cov(
        {
            ...
            "beta": params["beta"],
            "power": params["power"],
            "noise": jnp.zeros_like(params["noise"]),
        },
        freq,
        dose,
    )
